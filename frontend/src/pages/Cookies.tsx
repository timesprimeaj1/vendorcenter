...
const sectionKeys = ["functionalCookie", "outsideCookies", "whyWeUseThem"] as const;
        ...
        <div className="container relative py-16 md:py-24 text-center max-w-3xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/[0.08] backdrop-blur-md border border-white/[0.12] mb-6">
              ...
              <span className="text-xs font-semibold tracking-widest uppercase text-white/90">{t("cookies.badge")}</span>
            </div>
          </motion.div>
          ...
          {sectionKeys.map((key, i) => (
            <div key={key} className="p-6 rounded-2xl bg-card border border-border/60 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h2 className="font-display font-semibold text-lg mb-2">{t(`cookies.${key}`)}</h2>
                  <p className="text-muted-foreground leading-7">{t(`cookies.${key}Body`)}</p>
                </div>
              </div>
            </div>
          ))}
